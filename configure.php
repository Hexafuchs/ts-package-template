...
{
    $slug = $subject;
    if (str_ends_with($slug, '.js')) {
      $slug = basename($slug, '.js');
    }
    if (str_ends_with($slug, '.ts')) {
        $slug = basename($slug, '.ts');
    }
    return str_replace('-', '_', strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $slug), '-')));
}
...
$description = ask('Package short description', "This is my package {$packageName}");
